...
class triggerLoss(ABC):
    ...
    def _calculate_suspicious_loss(self, all_logits, batch, target_label):
        ...
        suspicious_probabilities = self._get_probabilitites(suspicious_logits,
                                                            batch=batch)
    ...
    def _calculate_clean_loss(self, all_logits, batch, target_label):
        agg_clean_probabilities = self._get_agg_clean_probabilities(all_logits,
                                                                        batch)
        target_probability_kwargs = {
            'batch': batch,
            'target_label': target_label}
    ...
    def _get_agg_clean_probabilities(self, all_logits, batch):
        probabilities = [self._get_probabilitites(self._get_logits(model_output), batch=batch) for model_output in all_logits['clean']]
