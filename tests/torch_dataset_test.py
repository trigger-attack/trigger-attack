...
class TestQAPreprocessing(unittest.TestCase):
    ...
    def setUp(self):
        ...
        trigger = Trigger(torch.tensor([0]*10),
                          location='both',
                          source_labels=None)
    ...
    def test_update_trigger(self):
        ...
        new_trigger = torch.tensor(list(
            range(len(self.preprocessor.trigger.input_ids))))
        dataset.update_trigger(new_trigger)
        input_ids = dataset[0]['input_ids']
        trigger_mask = dataset[0]['trigger_mask']
        self.assertTrue(torch.equal(
            input_ids[trigger_mask][:len(new_trigger)], new_trigger))
