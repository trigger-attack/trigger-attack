...
class TestSCPreprocessing(unittest.TestCase):
    ...
    def test_beginning_initialize_dummy_trigger(self):
        dataset = sc._tokenize_for_sc(self.dataset, self.models.tokenizer)
        dataset = tools._select_unique_inputs(dataset)
        trigger_length = 10
        trigger_loc = 'beginning'
        ...
        first_input_ids_without_dummy = self.dataset['input_ids'][0]
    ...
    def test_baseline_probabilities_no_grad(self):
        ...
        expected = torch.tensor([2.7970, -2.7824])
        actual = dataset['input_ids'][0].to('cpu')
